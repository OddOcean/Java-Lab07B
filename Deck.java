...
class Deck 
{
  ...
  public Deck(){
    int index = 0;//uses this variable to increment up cards array
    size = 52;
	 cards = new Card[size];//creates array to hold the cards
    ...
    for (int s = 0; s < suits.length; s++){//adds all cards of same suit before moving on to next suit
      for (int r = 0; r < ranks.length; r++){
        cards[index] = new Card(suits[s], ranks[r], values[r]);//adds card to array
        index++;
      }
    }
  }   
  ...
  public String toString(){
    String deck = "";
    for (int i = 0; i < cards.length - 1; i++){//adds whole deck to a string to be printed
      deck += cards[i] + ", ";
    }
